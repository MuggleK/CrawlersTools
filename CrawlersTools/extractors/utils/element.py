...
def calc_a_descendants_text_of_avg_length(element: Element):

    if element is None:
        return 0
    if element.tag.lower() == "a":
        if len(element.siblings) > 1:
            lengths = []
            for link in element.siblings:
                if element.attrib.get("title") is not None:
                    lengths.append(len(element.attrib.get("title")))
                if len(element.text.strip()) > 0:
                    lengths.append(len(element.text.strip()))
                if len(text(link)) > 0:
                    lengths.append(len(text(link)))
                lengths.append(0)
            if len(lengths) == 0:
                return 0
    ...
    for descentdant in siblings:
        if descentdant.number_of_a_char > 0:
            txt = descentdant.text
            regex = r"(2[0-9]{3}.?[0-1]{0,1}[0-9].?[0-3]{0,1}[0-9])"
            regex2 = r"([0-1]{0,1}[0-9]-[0-3]{0,1}[0-9])"
            time_match = len(re.findall(regex, txt)) or len(re.findall(regex2, txt))
            t = descentdant.number_of_a_char
            if time_match > 0:
                t = t + 10
            lengths.append(t / len(descentdant.a_descendants))
            continue
        if len(descentdant.text.strip()) > 0:
            lengths.append(len(descentdant.text.strip()))
            continue
        if len(text(descentdant)) > 0:
            lengths.append(len(text(descentdant)))
            continue
        lengths.append(0)
    if len(lengths) == 0:
        return 0
...
def number_of_p_descendants(element: Element):
    ...
    if element is None:
        return 0
    # for tag in ["p", "span", "tr", "td", "th", "u", "strong", "b", "section", "spanstyle", "spanlang"]:
    #     element_list.extend(element.xpath(f".//{tag}"))
